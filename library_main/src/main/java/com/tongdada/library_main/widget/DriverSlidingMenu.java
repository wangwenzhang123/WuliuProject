...
public class DriverSlidingMenu extends HorizontalScrollView {
    private static final float radio = 0.2f;//菜单占屏幕宽度比
    private final int mScreenWidth;
    private final int mMenuWidth;
    private boolean once = true;
    private boolean isOpen;
    public DriverSlidingMenu(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void onOpenMenu() {
        ...
        ((DriverManagerAdapter) ((RecyclerView) view).getAdapter()).holdOpenMenu(this);
        isOpen = true;
    }
    ...
    private void closeOpenMenu() {
        if (!isOpen) {
            ...
            ((DriverManagerAdapter) ((RecyclerView) view).getAdapter()).closeOpenMenu();
        }
    }
    long downTime = 0;
